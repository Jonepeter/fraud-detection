...
def add_transaction_features(df, user_id_col="user_id"):
    ...
    if user_id_col in df.columns and "purchase_time" in df.columns:
        ...
        #     df.groupby(user_id_col)["purchase_time"].diff().dt.total_seconds() / 60
        # )  # in minutes

        # Calculate average transaction value per user
        df["avg_user_purchase_value"] = df.groupby(user_id_col)[
            "purchase_value"
        ].transform("mean")
        ...
        df["purchase_value_ratio"] = (
            df["purchase_value"] / df["avg_user_purchase_value"]
        )
