...
class FraudDetectionPipeline:
    ...
    def run_fraud_data_pipeline(self):
        ...
        try:
            ...
            fraud_data, ip_data, _ = self.preprocessor.load_data(
                config.FRAUD_DATA_PATH, config.IP_COUNTRY_PATH, config.CREDITCARD_PATH
            )
            ...
            print(
                f"Balanced class distribution: {pd.Series(y_train_balanced).value_counts().to_dict()}"
            )
            ...
            explanation_dir = os.path.join(
                config.RESULTS_DIR, "fraud_data_explanations"
            )
    ...
    def run_credit_data_pipeline(self):
        ...
        try:
            ...
            explanation_dir = os.path.join(
                config.RESULTS_DIR, "credit_data_explanations"
            )
            importance_df = explainer.generate_explanation_report(explanation_dir)
...
def main():
    ...
    Runs the fraud and credit card data pipelines and generates the final report.
    Handles and prints any errors encountered during execution.
    """
    # Create directories
